...
export default class SceneManager {
  ...
  _init() {
    ...
    let _copy = `${process.env.REMOTE_ASSETS_DIR}assets/json/sculpture_assets.json?z=${Math.random()}`
    fetchJson(_copy).then(data => {
      let _sceneData = SceneData[this._sceneIndex]
      _sceneData.views = _.merge(_sceneData.views, data.views)
      _sceneData.textures = data.assets
      this._loadAssets()
    })
